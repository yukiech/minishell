...
static void	ft_single_start(t_command cmd, t_builtin *bt, char **envp);
...
void	ft_launch_commands(t_command *cmds, t_builtin *bt, char **envp, int nbcmd)
{
	if (nbcmd == 1)
	{
		ft_single_start(cmds[0], bt, envp);
	}
	else
	{
...
static void	ft_single_start(t_command cmd, t_builtin *bt, char **envp)
{
	...
	ft_make_dup(cmd.fdin, 0);
	ft_make_dup(cmd.fdout, 1);
	ft_search_command(cmd, bt, envp);
...
static void	ft_fork_commands(t_command *cmds, t_builtin *bt, char **envp, int nbcmd)
{
	...
	while (i < nbcmd)
	{
		...
		if (pids[i] == 0)
		{
			...
			ft_search_command(cmds[i], bt, envp);
			free(pids);
			ft_free_commands(cmds, nbcmd);
			ft_free_builtins(bt);
			exit(1);
